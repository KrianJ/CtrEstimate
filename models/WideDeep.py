...
class DeepLayer(Layer):
    ...
    def __init__(self, hidden_units, output_dim, activation):
        ...
        self.hidden_units = [Dense(i, activation=activation) for i in range(hidden_units)]
...
class WideDeep(Model):
    def __init__(self, dense_features, sparse_features, sparse_onehot_dim, sparse_embed_dim,
                 hidden_units, output_dim, activation):
        """
        WideDeep模型
        :param dense_features: 稠密数值特征名
        :param sparse_features: 稀疏类别特征名
        :param sparse_onehot_dim: 每个类别特征onehot后的维度
        :param sparse_embed_dim: 每个onehot类别特征转成embedding的维度
        ...
        self.embedding_layers = {
            'embed_{}'.format(i): Embedding(input_dim=sparse_onehot_dim[i],
    ...
    def call(self, inputs, training=None, mask=None):
        """inputs = [数值特征向量  label_encoding类别特征向量  onehot类别特征向量]"""
        ...
        # wide部分
        wide_input = tf.concat([dense_input, sparse_onehot_input])
